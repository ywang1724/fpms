                        ...
                        .exec(function (err, obj) {
                            if (err) {
                                ...
                                if (bookie.type !== 4) {
                                    ...
                                    Exception.findOne({
                                        page: bookie.page, type: bookie.type,
                                            ...
                                            if (!exception) {
                                                ...
                                                if (app.alarmtype.indexOf(bookie.type) !== -1) {
                                                    ...
                                                    new Exception({
                                                        page: bookie.page,
                                                        type: bookie.type,
                                                        errorurl: bookie.errorurl,
                                                        stack: bookie.stack,
                                                        message: bookie.message,
                                                        requrl: bookie.requrl,
                                                        lastAlarmTime: new Date(),
                                                        isAlarm: 1,
                                                        createTime: bookie.occurTime,
                                                        occurTimeAndUi: [{time: bookie.occurTime, ui: bookie.ui}]
                                                    }).save(function (err) {
                                                        if (err) {
                                                            console.log(errorHandler.getErrorMessage(err));
                                                        }
                                                ...
                                                if ((app.alarmtype.indexOf(bookie.type) !== -1) && (exception.isAlarm === 1) && ((new Date()).getTime() - exception.lastAlarmTime.getTime() >= app.alarmInterval )) {
                                                    ...
                                                    Exception.findOneAndUpdate({_id: exception._id}, {
                                                        lastAlarmTime: new Date(),
                                                        $push: {occurTimeAndUi: {time: bookie.occurTime, ui: bookie.ui}}
                                                    }).exec(function (err) {
                                                        if (err) {
        ...
        if (err) {
            ...
            for (var i = 0; i < exceptions.length; i++) {
                if (exceptions[i].occurTimeAndUi.length === 1) {
                    result.data.exceptions.push({
                        _id: exceptions[i]._id, createTime: exceptions[i].createTime,
            ...
            var promise1 = Exception.find({
                ...
                for (var i = 0; i < exceptions.length; i++) {
                    if (exceptions[i].occurTimeAndUi.length === 1) {
                        ...
                        for (var j = 0; j < exceptions[i].occurTimeAndUi.length; j++) {
                            tempExceptions.push({
                                _id: exceptions[i]._id,
                                createTime: exceptions[i].createTime,
                                occurTimeAndUi: exceptions[i].occurTimeAndUi[j],
                                lastAlarmTime: exceptions[i].lastAlarmTime,
                                page: exceptions[i].page,
                                type: exceptions[i].type,
                                errorurl: exceptions[i].errorurl,
                                stack: exceptions[i].stack,
                                message: exceptions[i].message,
                                requrl: exceptions[i].requrl,
                                isAlarm: exceptions[i].isAlarm
                            });
                        }
                    }
                }
...
exports.bookie = function (req, res) {
    var root = process.env.NODE_ENV === 'production' ? 'static/dist/' : 'static/js/'; // 文件路径
    ...
    App.findById(req.app._id, function (err, app) {
        if (app.config.exception) {
            //发送文件
            Q.all(['bookie.js', 'exception.js'].map(function (filename) {
                ...
                .then(function (data) {
                    var bookieJs = data[0].toString("utf8"),
                        exceptionJS = data[1].toString("utf8");
                    bookieJs = bookieJs.replace("{{'exception'}}", exceptionJS);
                    res.set("Content-Type", "text/javascript");
                    res.send(bookieJs);
                })
                .catch(function (err) {
                    res.send("console.log('" + err.toString() + "');");
                });
        } else {
            Q.Promise(function (resolve, reject) {
                fs.readFile(root + 'bookie.js', function (err, data) {
                    if (err) reject(err);
                    else resolve(data)
                })
            })
                .then(function (data) {
                    var bookieJs = data.toString("utf8");
                    bookieJs = bookieJs.replace("{{'exception'}}", "");
                    res.set("Content-Type", "text/javascript");
                    res.send(bookieJs);
                })
                .catch(function (err) {
